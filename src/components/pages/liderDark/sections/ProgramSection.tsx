        ...
        {programFeatures.map(({ icon: Icon, title, description, image, imageSm }) => (
          <StaggerItem key={title} as="li">
            ...
            >
              ...
              <div className="relative aspect-[4/3] w-[104px] shrink-0 overflow-hidden rounded-xl border border-white/[0.06] sm:aspect-video sm:w-full sm:rounded-none sm:border-x-0 sm:border-t-0">
                {image ? (
                  <img
                    src={image}
                  ...
                  <div
                    className="flex h-full items-center justify-center bg-gradient-to-br from-white/[0.06] via-transparent to-[#5E6AD2]/[0.06]"
                    aria-hidden
                  >
                    <Icon className="h-6 w-6 text-white/15 sm:h-7 sm:w-7" />
                  </div>
                )}
                {/* Мягкий стык полосы с текстом — только в вертикальной карточке */}
                <div
                  className="absolute inset-x-0 bottom-0 hidden h-16 bg-gradient-to-t from-linear-elevated/90 to-transparent sm:block"
                  aria-hidden
                />
              </div>
              ...
              <div className="flex min-w-0 flex-1 flex-col sm:p-6">
                <span
                  className="hidden h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/[0.04] text-linear-accent-bright transition-colors duration-300 ease-expo group-hover:border-[#5E6AD2]/30 group-hover:text-white sm:flex"
                  aria-hidden
                >
