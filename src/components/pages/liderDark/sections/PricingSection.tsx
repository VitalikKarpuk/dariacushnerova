          ...
          <div className="mt-8 gap-x-10 sm:columns-2 lg:columns-3">
            {includedGroups.map((group) => (
              <div key={group.label} className="mb-9 break-inside-avoid last:mb-0">
                ...
                <h4 className="border-b border-white/[0.06] pb-3 font-mono text-[11px] uppercase tracking-widest text-linear-fg">
                  {group.label}
                </h4>
                ...
                <ul className="divide-y divide-white/[0.06]">
                  {group.items.map(({ text, icon: Icon }) => (
                    <li key={text} className="flex items-start gap-3 py-3.5">
                      <Icon
                        className="mt-0.5 h-4 w-4 shrink-0 text-linear-accent-bright"
                        aria-hidden
                      />
                      <span className="min-w-0 flex-1 text-sm leading-relaxed text-linear-fg-muted">
                        {text}
                      </span>
                    </li>
                  ))}
      ...
      <Reveal delay={0.08} className="mt-4">
        ...
        >
          ...
          <div className="flex shrink-0 flex-col gap-2.5">
            ...
            <p className="flex items-center gap-2 text-xs text-linear-fg-muted">
